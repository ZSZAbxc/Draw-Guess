...
### 你画我猜模式（V2.5, 2026-08-02）

- 房间设置新增"你画我猜"模式开关（与默认模式平行，开启后隐藏灵机一动并强制关闭之）及说明文案；开启后"猜词时间"设置替换为"可选词数"（1-6，默认 3）。
- 回合流程：随机画手顺序 → 画手从 X 个候选词选 1 个（10s 超时自动选）→ 填提示（10s，≤5 字，留空确认=无提示）→ 广播"答案 N 个字+提示" → 画手作画（drawTime，画布固定 960x640 逻辑尺寸，增量笔画+5s 快照实时同步，跨设备按比例适配）→ 观众随时抢答（独立可滚动猜词框，默认显示最后几条，每轮清空；与上次猜测相同则忽略）→ 除画手外全猜对或作画超时结束 → 20s 相符/不相符评价（全员投完提前 5s）→ 下一轮 → 所有人轮完结算。
- 计分：第 1 个猜对者得（人数-1）分，每往后一名 -1，最低 1 分；画手获得与每个猜对者相同分数；每张相符票画手 +1。结算含"一眼丁真"（猜手榜）、"灵魂画手"（画手榜）、"MVP"（总分）。
- 画手断线：不结束本轮，时间照常推进（选词/提示/作画超时兜底），重连通过 `ydig_state` 恢复（含画布快照与猜词历史）。
- 重开安全：抽出 `resetRoomAfterGame` 统一重置并**完整保留设置**（修复了普通模式结算后 `youDrawIGuess/wordCount` 被丢弃的问题）；`wordCount` 开局冻结；E2E 实测 ydig→经典+灵机一动、ydig→ydig 同模式重开（可选词数保留）、经典回归全部正常。
- 本地 E2E 52 项断言全过；生产冒烟 9/9 通过。
...
- **V2.5 部署（2026-08-02）**：SFTP 上传 `server.js`、`public/game.js`、`public/index.html`、`public/style.css`（备份 `*.bak-20260802-174045`），重启后服务 active，本地 8080 与 nginx `/draw-guess/` 均 200；生产你画我猜冒烟 9/9（建房/模式/选词/提示/实时笔画/猜词计分/两轮/结算含MVP）；sha1 与本地一致。
